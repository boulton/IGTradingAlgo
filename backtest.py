...
class histdataconverter(object):
	...
	def csvreader(self, arg, spread): # cleaned
		...
		sample0 = {"snapshotTimeUTC":[],"openPrice":[], "highPrice":[],"lowPrice":[],"closePrice":[],"lastTradedVolume":[]}
		with open("archive/histdata/{}.csv".format(arg), 'r+', encoding='utf-8') as outfile:
			reader= csv.reader(outfile, delimiter=';')
			for row in reader :
				#  date brut : "20180101 170000"
				tl = ([int(row[0][:4]), int(row[0][4:6]), int(row[0][6:8]), int(row[0][9:11]), int(row[0][11:13]),int(row[0][13:15])])
	...
	def pricediscovery(self, sample): #cleaned
		...
		for x in range(1, len(sample["snapshotTimeUTC"]) - 1):
			...
			rest.indicateur.indicateur(temps=temps, Midpoint=((bid+ask)/2))
			rest.strategie.strategie(BidAsk=[bid,ask],Price_0=[bidbefore,askbefore], temps=temps, backtest=Var.backtest)
	...
	def toDataframe(self, sample):
		...
		logger.debug("Time:{} close,{} ma:{} autre: {}".format(len(df["Time"]), len(df["Trade2"]), len(df["MA"]), len(df["bidClose"])))
