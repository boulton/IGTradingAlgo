...
class LSClient(object):
    ...
    def subscribe(self, subscription):
        ...
        # StartTime sert au minuteur/chrono dans indicateur
        Var.lastMinute = int(datetime.datetime.now().minute)
...
def streaming(data):
    ...
    Var.price["Date"] = (datetime.datetime.now(datetime.timezone.utc)).date().replace('-', '/')
    #  a new connection to Lightstreamer Server
...
def archive():
    ...
    for x in range(1, len(Var.price["Time"]) - 1):
        ...
        ArchStream +=  "," #+nonArchiver(i) # BROKEN
    ...
    try:  # x pour create rights only
        open("Data/" + titre + '.json', 'x')
    except FileExistsError as e:
        pass  # <--warning/log

    with open("Data/" + titre + '.json', 'r+') as out:
        a = out.read()
        out.seek(0)
