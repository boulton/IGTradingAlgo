...
class strategie():
	...
	def __init__(self, BidAsk, temps, Price_0, backtest):
		...
		self.SimpleEmaCross0(BidAsk, Price_0, backtest, temps)
	...
	def SimpleEmaCross0(self, BidAsk, Price_0, backtest, tempo):
		...
		condition1 = initparam and c1
		condition2 = initparam and c2
	...
	def SimpleEmaCross(self, BidAsk, Price_0, backtest, tempo):
		...
		if condition1:
			...
			if backtest is False:
				rest.ig().CreateOrders(orderType="MARKET", direction=sens, stopDistance=Var.Stoploss, limitDistance=self.limit, epic=Var.epic)
			...
			if backtest is False:
				rest.ig().CreateOrders(orderType="MARKET", direction=sens, limitDistance=self.limit, stopDistance=self.stoploss, epic=Var.epic)
				# redondance rest.ig()
			else:
	...
	def event(self, backtest, sens, tempo, BidAsk, Price_0):
		...
		if backtest:
			...
			if not maxposition:
				rest.ig().CreateOrders(orderType="MARKET", direction=sens, stopDistance=Var.Stoploss, limitDistance=Var.Limit, epic=Var.epic)
			# rest.ig() crée une nouvelle instance -> overload de connection
		Var.ordrePassee = True
	...
	def profit(self, dico,i, BidAsk ):
		...
		if sens =="SELL":
			prixactuel = float(ask)
			profit = round((prixentree - prixactuel) * 10000, 2)
		else: 
			prixactuel = float(bid)
			profit = round((prixactuel - prixentree) * 10000, 2)
