...
class ig:
	...
	def Auth(self, login, password):
		...
		payloads = {"identifier": login ,
					"password": password }
		headers = {'X-IG-API-KEY': "YOUR-IG-API",
				   'Version': '3'}
	...
	def streamingToken(self, arg):
		...
		h1 = {'IG-ACCOUNT-ID': data.get('accountId'),
			  ...
			  'Version': '1', 'X-IG-API-KEY': "YOUR-IG-API" 
			  }
	...
	def IgClientPositions(self,epic):
		...
		""" Pas de donnée avant le else d'autentification"""
		if not self.authentified :
			self.Auth(Var.login,Var.password)
			return self.IgClientPositions(epic)
		else :
	...
	def retrieve(self): 
		...
		h1 = {'IG-ACCOUNT-ID': self.data.get('accountId'), 'Authorization': self.data.get('oauthToken').get('token_type') + " " + self.data.get(
			'oauthToken').get('access_token'), 'Version': '3', 'X-IG-API-KEY': "YOUR-IG-API"}
...
def CloseFakeOrders(Dealtuple):
	...
	for x in range(len(Dealtuple)):
		...
		profit = round((float(Var.price[j][currentiter]) -float(entry))*100000,2)
		Var.PnL =+ profit
		...
		print("Deal "+Var.FakeDeal["Id"][i]+" CLOTURE !")

		Var.FakeDeal["Time"].pop(i)
